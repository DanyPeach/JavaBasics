...
Дефолтный метод - нужен для добавления функциональности в
интерфейс без необходимости изменять все его имплементации. То есть
сразу будет реализация и не обязательно менять ее. Если все методы
делают одно и тоже, то зачем в каждом классе делать одно и тоже

Приватный метод - нужен для локализации реализации и скрытия деталей
реализации от имплементаций интерфейса. Если дефолтный метод огромный
то легче разбить его на несколько мелких - приватных

Статический метод - нужен для предоставления функциональности, которая не зависит от
конкретной имплементации интерфейса. Он может использоваться
для утилитарных методов, констант или фабричных методов.
то есть это вспомогательные методы
     */
}
